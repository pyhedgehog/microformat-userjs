...
function SqliteCache(dbfn) {
  ...
  var onDbError = this._db_onerror.bind(this);
  this.db.on('error', onDbError);
  this.size = null;
  var self = this;
  process.nextTick(function() {
    ...
    .then(function() {
      ...
      .then(function() {
        self.fetch_list(function() { self.emit('ready'); });
      });
    });
  });
}
...
SqliteCache.prototype.ready = function(cb, fb) {
  //console.log('SqliteCache.prototype.ready');
  if(!this.db) {
    if(fb) { fb(this.error); }
    return this;
  }
...
SqliteCache.prototype._db_onerror = function(err) {
  //console.log('SqliteCache.prototype._db_onerror');
  this.error = err;
  this.db.close();
  this.db = null;
  this.emit('end');
