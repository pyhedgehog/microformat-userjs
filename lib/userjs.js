...
function resourceLoader(resource, callback, cache) {
  ...
  if(cache)
    return cache.get(resource.url.href, callback, resourceLoader_nocache);
  return resourceLoader_nocache();
  function resourceLoader_nocache(err) {
...
MicroformatNodeEngine.prototype.array = function(document, window) {
  ...
  this.module.parseDom(jDom, jRoot, cfg, function(err, data) {
...
UserJSApply.prototype.userjs_queue = function UserJSApply_userjs_queue() {
...
UserJSApply.prototype.userjs_onend = function UserJSApply_userjs_onend() {
